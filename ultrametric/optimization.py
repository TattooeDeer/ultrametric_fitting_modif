...
class UltrametricFitting:
    ...
    def __init__(self, epochs, lr, loss, space_transformer_params, projection='soft', early_stop=True, ultrametric_projection=subdominant_ultrametric):
        """
        ...
        self.max_loss = 0
        self.gradients_dict = {}
        self.space_transformer_params = space_transformer_params
    ...
    def _setup(self, graph, edge_weights, init=None):
        ...
        self.optimizer = tc.optim.Adam([self.M, *self.space_transformer_params], lr=self.lr, amsgrad=True)
        self.history = []
        
    def _optimization(self):
        print('UltrametricFitting...')
        for t in range(self.epochs):
            print(f'epoch {t}')
            self.optimizer.zero_grad()
            loss = self._criterion()
            loss.backward()
            for name, param in self.space_transformer.named_parameters():
              if 'weight' in name:
                temp = tc.zeros(param.grad.shape)
                temp[param.grad != 0] += 1
                self.gradients_dict[name] += temp
            self.optimizer.step()            
